...
/** Fixed-size executor configuration used for the scheduler, parallel, and async pools. */
...
/** Full runtime configuration for [[ParIORuntime]]. */
...
/** Configurable runtime/interpreter for [[ParIO]].
  *
  * `ParIO` itself remains a pure program data type. A [[ParIORuntime]] supplies the actual executors plus the
  * [[Effect]] / [[Parallel]] instances used by parapet to interpret those programs.
  */
final class ParIORuntime(val config: ParIORuntimeConfig) extends AutoCloseable:
  ...
  /** [[Effect]] instance backed by this runtime's pools. */
  ...
  /** [[Parallel]] instance backed by this runtime's dedicated parallel pool. */
  given parallel: Parallel[ParIO] with
    def par(effects: Seq[ParIO[Unit]]): ParIO[Unit] =
      ParIO.delay(runParallel(effects))

  /** Internal scheduler-worker capability backed by this runtime's dedicated scheduler pool. */
  ...
  /** Interpret `fa` synchronously on the current thread.
    *
    * Package-private so applications enter through `ParIO.unsafeRunSync` / `ParIOApp` rather than directly driving a
    * runtime instance.
    */
