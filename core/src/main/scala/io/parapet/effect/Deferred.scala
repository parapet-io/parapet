...
/** A "set once, await many" cell used to bridge synchronous code with the effect type `F`.
  *
  * Backed by a JDK [[CompletableFuture]] — readers wait via [[get]], writers fulfill via
  * [[complete]]. Subsequent completions are no-ops (the underlying future is set-once).
  */
