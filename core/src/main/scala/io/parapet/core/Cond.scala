...
/** A one-shot conditional waiter implemented as a [[Process]].
  *
  * The originating sender issues a [[Cond.Start]] event; the `Cond` then watches all
  * subsequent events and replies once with a [[Cond.Result]] containing the first event
  * for which `predicate` returned `true`. If no matching event arrives within `timeout`
  * the result is `None`.
  *
  * Useful for "wait until X happens" patterns without manually wiring callbacks.
  *
  * @param predicate decides whether an incoming event satisfies the condition.
  * @param timeout   maximum time to wait for a satisfying event before responding with
  *                  `Result(None)`.
  */
...
object Cond:
  ...
  /** Reply emitted exactly once when the condition fires or the timeout elapses.
    *
    * @param event `Some(event)` if a matching event arrived, `None` on timeout.
    */
