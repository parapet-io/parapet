...
/** Spillover sink for envelopes that the runtime cannot deliver because a process's mailbox is full.
  *
  * Implementations may persist to disk, replay later, or simply log the drop. The default is [[EventStore.Stub]] which
  * logs at debug.
  */
...
object EventStore:
  /** No-op [[EventStore]]. Logs each dropped envelope at debug and never returns anything from [[read]]. Suitable for
