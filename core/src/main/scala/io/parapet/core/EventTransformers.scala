...
/** Immutable registry of [[EventTransformer]]s keyed by [[ProcessRef]].
  *
  * Looked up by [[DslInterpreter]] for every send so that transformers can be applied to
  * the targeted process's incoming events.
  *
  * @param transformers map from receiver ref to its transformer.
  */
