...
/** Bootstraps a parapet application on top of an arbitrary effect type `F[_]`.
  *
  * `ParApp` is the user-facing entry point: subclass it, override [[processes]] to return the application's actor
  * population, and call [[main]] (or [[run]]) to start the runtime.
  *
  * The class wires together the supporting machinery - [[Context]], [[Scheduler]], [[DslInterpreter]],
  * [[EventTransformer]] pipeline, and [[DeadLetterProcess]] - so subclasses normally only need to express their domain
  * logic.
  *
  * Concrete effect implementations are exposed via dedicated subclasses such as [[ParIOApp]]. Users targeting a
  * different effect type implement their own subclass providing the [[effectInstance]] / [[parallelInstance]]
  * capabilities and an [[unsafeRun]] entry point.
  *
  * @tparam F
  *   the effect type used to express asynchronous and side-effecting computations.
  */
trait ParApp[F[_]] extends FlowSyntax[F]:
  /** The [[Effect]] type-class instance for `F`. Subclasses provide this to wire the runtime against their chosen
    * effect monad.
    */
  protected def effectInstance: Effect[F]

  /** The [[Parallel]] type-class instance for `F`, providing fork/join primitives needed by the scheduler.
    */
  ...
  /** Boots the runtime end-to-end: builds the [[Context]] and [[Scheduler]], registers the user processes plus the
    * dead-letter handler, and starts dispatching.
    *
    * The returned `F[Unit]` completes when the scheduler returns control - typically once `Stop` propagates. Use
    * [[main]] for a blocking entry point.
    *
