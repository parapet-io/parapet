...
class JournalStoreLocalSpec extends AnyFunSuite:
  ...
  test("read merges and sorts across batches, including out-of-order ranges") {
    val (_, store) = newStore()
    store.append(Vector(entry(5), entry(7))).unsafeRun()
    store.append(Vector(entry(3), entry(4), entry(6))).unsafeRun()
  ...
  test("truncate drops segments fully at or below upToSeq, keeps the rest") {
    val (_, store) = newStore()
    store.append(Vector(entry(1), entry(2))).unsafeRun() // [1,2]
    store.append(Vector(entry(3), entry(4))).unsafeRun() // [3,4]
    store.append(Vector(entry(5), entry(6))).unsafeRun() // [5,6]
  ...
  test("truncate keeps a segment straddling upToSeq whole") {
    val (_, store) = newStore()
    store.append(Vector(entry(3), entry(7))).unsafeRun() // [3,7]
