...
/** A multicast/filter process: re-publishes every non-system event it receives to a fixed set of subscribers,
  ...
  *
  * Useful as the receiving end of a fan-out where multiple consumers need to observe the same event stream without the
  * producers knowing about them.
  *
  * @param subs
...
object Sub {
  ...
  /** A single subscription.
    *
    * @param ref
    *   process to forward matching events to.
    * @param filter
