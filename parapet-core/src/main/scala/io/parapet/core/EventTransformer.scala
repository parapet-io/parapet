...
/** Transforms events on their way to a specific receiver.
  *
  * Registered per [[io.parapet.ProcessRef]] via [[io.parapet.ParApp.eventTransformer]]; applied by the
  * [[DslInterpreter]] just before an envelope is enqueued.
  *
  * Useful for cross-cutting concerns such as schema migration, encryption, or trace propagation. Events outside `f`'s
  * domain pass through unchanged.
  *
  * @param f
  *   partial function mapping input events to their transformed form.
  */
