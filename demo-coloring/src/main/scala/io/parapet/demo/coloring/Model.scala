...
/** Per-node state surfaced over the JSON API to the web UI.
  *
  * @param x,y           initial layout hints; the front-end uses these to seed the
  *                      3D force layout.
  * @param neighbors     ids of adjacent nodes (graph topology).
  * @param status        node lifecycle, see [[ColoringNodeStatus]].
  * @param color         current color (when locked or proposing successfully).
  * @param proposedColor color tentatively proposed during a coloring round.
  * @param conflict      whether the most recent proposal collided with a neighbour.
  * @param clusterId     cluster the node belongs to (clusters spawn over time).
  * @param conquests     number of times the node has been wounded/flipped in battle;
  *                      reaching `MaxConquests` permanently locks it.
  */
...
/** Event emitted by the simulation; consumed by the front-end event log.
  *
  * @param tick   monotonically-increasing simulation tick.
  * @param kind   event category (e.g. `"propose"`, `"conquest"`, `"siege"`).
  * @param nodeId optional subject node id.
...
/** Complete simulation state; the JSON serialisation of this object drives the UI.
  *
  * @param graphId      identifier of the current graph generation; bumped when the
  *                     user resets/regenerates so the front-end can reset its caches.
  * @param round        coloring/battle round counter.
  * @param tick         monotonic event tick (separate from `round`).
  * @param nodeCount    total nodes currently in the graph.
  * @param paletteSize  number of available colors.
  * @param canvasWidth  layout viewport width in pixels.
  * @param canvasHeight layout viewport height in pixels.
  * @param running      whether the background simulation loop is active.
  * @param completed    whether the current mode has terminated.
  * @param nodes        per-node state.
  * @param cluster      Raft cluster overlay (may be empty).
  * @param events       recent simulation events (capped to a sliding window).
  * @param clusters     summary metadata about node clusters.
  * @param mode         active game mode.
  * @param races        per-color population stats in [[GameMode.Battle]].
  * @param victor       winning color when a battle has terminated.
  */
