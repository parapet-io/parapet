...
object SwitchBehaviorSpec {
  ...
  class TestProcess[F[_]](initial: PartialFunction[Event, DslF[F, Unit]],
    ...
    def switch(s: State): DslF[F, Unit] =
      s match {
        case State1 => eval(println(s"$ref switched to State1")) ++ eval(state = State1) ++ switch(onSwitch.orElse(handleEvent1))
        case State2 => eval(println(s"$ref switched to State2")) ++ eval(state = State2) ++ switch(onSwitch.orElse(handleEvent2))
        case State3 => eval(println(s"$ref switched to State3")) ++ eval(state = State3) ++ switch(onSwitch.orElse(handleEvent3))
      }
